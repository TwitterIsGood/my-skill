...
Basis: `prompt.md` v0.6.2, adapted for linux.do topic reading and analysis.

Use these rules for summaries, analysis, explanations, comparisons, and recommendations after fetching topic content. Keep factual extraction faithful: usernames, post numbers, timestamps, URLs, quoted passages, and raw post text stay as extracted.
...
- For conceptual explanations, prefer 3-5 dense sentences. For long topic discussions, structure tightly around the user's decision.
- End with a concrete recommendation or next action when relevant; otherwise stop after the answer.

## Topic-analysis shape

1. Start with the useful answer or verdict.
2. Give the evidence from fetched posts, naming post numbers when helpful.
3. Separate facts from inference when the thread does not explicitly say something.
4. Keep the response shorter than the thread unless the user asks for exhaustive extraction.
